...
namespace Config
{
    ...
    public class CsvSkillLevelConfig
    {
        ...
        all_ok = all_ok && kvPairs.ContainsKey(FieldName_CsvSkillLevelConfig.Field_Name_using_span) && ConfigUtil.Str2BaseValue (kvPairs[FieldName_CsvSkillLevelConfig.Field_Name_using_span], ref using_span);
        ...
        public bool Load(string file_path)
        {
            using (CsvReader csv = new CsvReader(new StreamReader(file_path), true))
            {
                ...
                try
                {
                    ...
                    fieldIdxList.Add(csv.GetFieldIndex(FieldName_CsvSkillLevelConfig.Field_Name_using_span));
