...
public class Moba_Camera_Boundaries
{
	...
	private List<Moba_Camera_Boundary> cube_boundaries 	= new List<Moba_Camera_Boundary>();
	private List<Moba_Camera_Boundary> sphere_boundaries = new List<Moba_Camera_Boundary>();
	...
	public bool AddBoundary(Moba_Camera_Boundary boundary)
    {
		if(boundary == null)
        {
	...
	public void RemoveBoundary(Moba_Camera_Boundary boundary)
    {
        cube_boundaries.Remove(boundary);
        sphere_boundaries.Remove(boundary);
	}

    public void ClearAllBoundaries()
    {
        cube_boundaries.Clear();
        sphere_boundaries.Clear();
    }	
	
	// Check if a give point in within any boundary contained in the list
	public bool isPointInBoundary(Vector3 point) {
	...
	public Moba_Camera_Boundary GetClosestBoundary(Vector3 point) {
