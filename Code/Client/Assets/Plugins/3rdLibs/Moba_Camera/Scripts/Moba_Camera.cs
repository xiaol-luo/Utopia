...
public class Moba_Camera : MonoBehaviour {
    ...
    // Bondaries
    Moba_Camera_Boundaries m_camera_boundaries = new Moba_Camera_Boundaries();
    public Moba_Camera_Boundaries cameraBoundaries { get { return m_camera_boundaries; } }
	...
	void CalculateCameraBoundaries() {
			...
			(Input.GetButton(inputs.axis.button_camera_move_right)))) 
		{
			...
			if(!m_camera_boundaries.isPointInBoundary(requirements.pivot.position)) {
				// Get the closet boundary to the pivot
				Moba_Camera_Boundary boundary = m_camera_boundaries.GetClosestBoundary(requirements.pivot.position);
				if(boundary != null) {
