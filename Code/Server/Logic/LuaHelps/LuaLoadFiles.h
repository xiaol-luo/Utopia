...
namespace LuaUtils
{
	...
	const static char * Load_Files_Effect_Script_Table_Name = "effect_script";
	...
	bool LoadScripts_DoLoadScript(bool isReload, std::set<std::string> reloadFilter)
	{
		...
		sol::state_view lsv(LuaUtils::luaState);
		sol::table luaFiles = lsv[Load_Files_Table_Name];
		if (!luaFiles.valid())
		{
		...
		for (auto item : luaFiles)
		{
			if (isReload)
			{
				...
				if (reloadFilter.end() == reloadFilter.find(name))
					continue;
			}			
